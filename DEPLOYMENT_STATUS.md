...
├── README.md             (created)
├── subgraph.yaml         (fixed network + address + events)
├── schema.graphql        (simplified to match contract)
├── src/mapping.ts        (rewritten for ExperimentRegistered)
├── abis/
│   └── PoXRegistry.json  (copied from out/)
├── generated/            (auto-generated by codegen)
└── build/                (WASM output from build)
```
