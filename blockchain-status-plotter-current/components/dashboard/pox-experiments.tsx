...
import { RefreshCw, Beaker, Wind, Database, Atom, Shield, AlertCircle } from "lucide-react"
...
export function PoXExperiments({ initialExperiments, initialStats }: PoXExperimentsProps) {
  ...
  async function refreshData() {
    ...
    try {
      ...
      console.error('[Dashboard] Failed to refresh experiments:', error)
      
      // User-friendly error messages
      if (error.name === 'TimeoutError' || error.name === 'AbortError') {
        setError('Request timeout. Network may be slow.')
      } else if (error.message.includes('Failed to fetch')) {
        setError('Network error. Check your connection.')
      } else {
        setError('Failed to load experiments. Showing cached data.')
      }
    } finally {
...
// Memoized experiment card to prevent unnecessary re-renders
const ExperimentCard = memo(function ExperimentCard({ experiment }: { experiment: UnifiedExperiment }) {
